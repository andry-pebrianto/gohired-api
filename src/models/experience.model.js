...
module.exports = {
  ...
  addExperience: (data) => new Promise((resolve, reject) => {
    const {
      id, position, photo, company, start_date, end_date, description, userId, createdAt,
    } = data;
    ...
    db.query(
      ...
      [id, position, photo, company, start_date || null, end_date || null, description, userId, createdAt],
      (error, result) => {
        if (error) {
          reject(error);
        }
